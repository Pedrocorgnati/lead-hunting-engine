...
const TEST_EMAIL = process.env.E2E_LOCKOUT_TEST_EMAIL ?? 'lockout-test@lead-hunting-engine.test'
const WRONG_PASSWORD = 'wrong-password-for-lockout-test'

async function attemptLogin(request: APIRequestContext, password = WRONG_PASSWORD) {
...
test.describe('Auth lockout (exponential backoff)', () => {
  test('primeiras 3 tentativas falham com 401, nao 429', async ({ request }) => {
    for (let i = 0; i < 3; i++) {
      const res = await attemptLogin(request)
  ...
  test('a partir da 4a tentativa recebe 429 com Retry-After crescente', async ({ request }) => {
    ...
    const res4 = await attemptLogin(request)
    ...
    const res5 = await attemptLogin(request)
  ...
  test('sucesso com senha correta reseta o contador', async ({ request }) => {
    ...
    // Forcar 3 falhas
    for (let i = 0; i < 3; i++) {
      await attemptLogin(request, WRONG_PASSWORD)
    }

    // Login com senha correta deve ter sucesso
    const resOk = await attemptLogin(request, CORRECT_PASSWORD)
    expect(resOk.status()).toBe(200)

    // Proxima tentativa errada deve comecar do zero (401, nao 429 imediato)
    const resAfter = await attemptLogin(request, WRONG_PASSWORD)
